...
describe('createRepeat', () => {
  ...
  const tests: Tests = [
    ...
    {
      expectation: 'should emit the source stream value only once with the global count set to 1',
      input: { globalCount: 1, emitCount: 2 },
      expected: [sourceValue],
    },
    {
      expectation: 'should emit the source stream value twice with the global count set to 2',
      input: { globalCount: 2, emitCount: 2 },
      expected: [sourceValue, sourceValue],
    },
    {
      expectation: 'should emit the source stream value twice with the count set to 2',
  ...
  tests.forEach(({ expectation, input: { globalCount, count, emitCount }, expected }) =>
    it(expectation, () =>
  ...
  it('should throw an error when it is invoked outside an injection context', () => expect(createRepeat).toThrow());
});
