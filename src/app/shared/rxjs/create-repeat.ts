...
// Personnal tool made for ngxtension: ongoing PR on https://github.com/ngxtension/ngxtension-platform/pull/380/files
export function createRepeat(destroyRef?: DestroyRef): CreateRepeat;
export function createRepeat(generalCount?: number, destroyRef?: DestroyRef): CreateRepeat;
export function createRepeat(generalCountOrDestroyRef?: number | DestroyRef, destroyRef?: DestroyRef) {
  ...
  const repeatFn = <T>(count?: number) => repeat<T>({ count: count ?? generalCount, delay: () => repeat$ });
...
function parseArgs(generalCountOrDestroyRef?: number | DestroyRef, destroyRef?: DestroyRef) {
